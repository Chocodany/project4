...
public class Blocks implements Comparable<Blocks> {
	...
	public void readTransactions(String filename)throws IOException{
		...
		String[] inputs;
		TreeSet<Transaction> tranSet = new TreeSet<Transaction>();
		
		while (scnr.hasNextLine()) {
		inputs = scnr.nextLine().split(",");
		int num = Integer.parseInt(inputs[3]);
		int index = Integer.parseInt(inputs[4]);
		int gasLimit = Integer.parseInt(inputs[8]);
		long gasPrice = Double.valueOf(inputs[9]).longValue();
		String fromAdr = inputs[5];
		String toAdr = inputs[6];
		
		Transaction trial = new Transaction(num, index, gasLimit, gasPrice, fromAdr, toAdr);
		tranSet.add(trial);
		}
		
		scnr.close();
		transactions.addAll(tranSet);
		tranSet.clear();
	}
	...
	public double transactionCost() {
		return getTransactionCount() * getNumber() / 1E+18;
		
	}
	
	public void uniqFromTo() {
		Boolean isUnique;
		ArrayList<Transaction> newString = new ArrayList<Transaction>();
		double total;
		System.out.println("Each transaction by from address for Block " + this.getNumber() + ":");
		
		for(int i = 0; i < transactions.size(); i++){
			ArrayList<Transaction> other = new ArrayList<Transaction>();
			isUnique = true;
			total = 0;
			
			for (int t = 0; t < other.size(); t++) {
				for (int c = 0; i < other.size()-1;c++) {
					int g = other.get(i).compareTo(other.get(i+1));
					if (g > 0) {
						Transaction b = other.get(c);
						other.set(c, other.get(c+1));
						other.set(c+1, b);
					}
				}
			}
			if (!(i==0)) {
				...
				System.out.println();
				for (int t = 0; t < other.size(); t++) {
					System.out.println(" -> " + other.get(t).getToAddress());
				}
				System.out.printf("Total cost of transaction: %.8f ETH", total);
				newString.add(transactions.get(i));
			}
		}
